...
int icmp_unpack(char *buf, int len)
{
	...
	{
		fprintf(stderr,
			"Invalid icmp packet.Its length is less than 8\n");
		return -1;
	}
	if ((icmp->icmp_type == ICMP_ECHOREPLY)
	    && (icmp->icmp_id == (pid & 0xffff))) {
		if ((icmp->icmp_seq < 0)
		    || (icmp->icmp_seq > PACKET_SEND_MAX_NUM)) {
		...
		printf("%d byte from %s: icmp_seq=%u ttl=%d rtt=%d ms\n",
		       len, inet_ntoa(ip_hdr->ip_src), icmp->icmp_seq,
		       ip_hdr->ip_ttl, rtt);

	} else {
		fprintf(stderr,
			"Invalid ICMP packet! Its id is not matched!\n");
		return -1;
	}
	return 0;
}
...
void ping_send()
{
	...
	while (alive) {
		...
		size =
		    sendto(rawsock, send_buf, 64, 0, (struct sockaddr *)&dest,
			   sizeof(dest));
...
void ping_recv()
{
	...
	while (alive) {
		...
		default:
			{
				int size =
				    recv(rawsock, recv_buf, sizeof(recv_buf),
					 0);
				if (size < 0) {
...
void ping_stats_show()
{
	...
	printf
	    ("%d packets transmitted, %d recieved, %d%c packet loss, time %ldms\n",
	     ...
	     (send_count - recv_count) * 100 / send_count, '%', time);
}
